...
### visualize velocity on t-SNE embedding
cell.colors_fullseq2 <- gsub("#00FF80FF", "#CCCCCC", gsub("#00FF00FF", "#006600", cell.colors_fullseq))

show.velocity.on.embedding.cor(emb,rvel.cd,n=200,scale='sqrt',cell.colors=ac(cell.colors_fullseq2,alpha=0.5),cex=0.8,arrow.scale=3,show.grid.flow=TRUE,min.grid.cell.mass=0.5,grid.n=40,arrow.lwd=1,do.par=F,cell.border.alpha = 0.1)

## new function to generate generate summary count plot with color matching t-SNE

#cell.colors_fullseq2 <- gsub("#00FF80FF", "#CCCCCC", gsub("#00FF00FF", "#006600", cell.colors_fullseq2))

plotCountSummary <- function(input, color_file){
  cell_type <- input %>% group_by(V2) %>% summarise(count = n()) 
  cell.colors_fullseq_df <- as.data.frame(color_file)
  cell_color <- cell.colors_fullseq_df %>% group_by(color_file) %>%
    summarise(count = n())
  ...
  cell_type_color %>%
    ...
    scale_fill_manual(values =  as.character(unlist(cell_type_color$color_file))) + geom_text(aes(label=count))+
...
########################### old ########################################################### 
## fig2
## manually assign 12 color
cell.colors_fullseq2 <- gsub("#00FF80FF", "#CCCCCC", gsub("#00FF00FF", "#006600", cell.colors_fullseq2))


show.velocity.on.embedding.cor(emb,rvel.cd,n=200,scale='sqrt',cell.colors=ac(cell.colors_fullseq2),
                               cex=0.8,arrow.scale=3,show.grid.flow=TRUE,
                               min.grid.cell.mass=0.5,grid.n=40,arrow.lwd=1,do.par=F,
                               cell.border.alpha = 0.1)

## construct legend
cell.colors_fullseq2_df <- as.data.frame(cell.colors_fullseq2) 
cell.colors_fullseq2_df <- cell.colors_fullseq2_df%>%
  group_by(cell.colors_fullseq2) %>% 
  summarise(n=n()) %>%
  as.data.frame() %>%
  .[order(.$n),]

cell.colors_fullseq2_name <- fullseq_label %>%
  group_by(V2) %>%
  summarise(n = n()) %>%
  as.data.frame() %>%
  right_join(cell.colors_fullseq2_df, by = "n")

p <- show.velocity.on.embedding.cor(emb,rvel.cd,n=200,scale='sqrt',cell.colors=ac(cell.colors_fullseq2),
                               cex=0.8,arrow.scale=3,show.grid.flow=TRUE,
                               min.grid.cell.mass=0.5,grid.n=40,arrow.lwd=1,do.par=F,
                               cell.border.alpha = 0.1)
show.velocity.on.embedding.cor(emb,rvel.cd,n=200,scale='sqrt',cell.colors=ac(cell.colors_fullseq2),
                               ...
                               min.grid.cell.mass=0.5,grid.n=40,arrow.lwd=1,do.par=F,
                               cell.border.alpha = 0.1, 
                               scale_fill_manual(values = cell.colors_fullseq2_name$cell.colors_fullseq2,
                                                 labels = cell.colors_fullseq2_name$V2))
scale_fill_manual(values = c(Other_Immune = "#0000FFFF"))

## adjust factor level
cell.colors_fullseq2_name$V2 <- factor(cell.colors_fullseq2_name$V2, 
                                          levels = c("Other_Immune", "Langerhan_cell", "T_cell",
                                                     "K14_Prolif", "Bulge", "DP", "Upper_HF_2",
                                                     "Fibroblast", "Upper_HF_1", "Krt14_Basal_2",
                                                     "Krt1_Spinous", "Krt14_Basal_1"))
cell.colors_fullseq2_name$cell.colors_fullseq2<-factor(cell.colors_fullseq2_name$cell.colors_fullseq2,
                                                       levels = c("#0000FFFF", "#0080FFFF", "#8000FFFF", "#80FF00FF", "#FF0000FF",
                                                                  "#FF8000FF", "#FF0080FF", "#FFFF00FF", "#FF00FFFF", "#00FFFFFF", "#006600",
                                                                  "#CCCCCC"))
cell.colors_fullseq2_name %>% ggplot(aes(x=V2, y = n, fill=V2)) + 
  geom_bar(stat="identity") + 
  scale_fill_manual(values =  c("#0000FFFF", "#0080FFFF", "#8000FFFF", "#80FF00FF", "#FF0000FF",
                                "#FF8000FF", "#FF0080FF", "#FFFF00FF", "#FF00FFFF", "#00FFFFFF", 
                                "#006600","#CCCCCC")) + geom_text(aes(label=n))+
  xlab("cell type") + ylab("counts")+
  theme(axis.text.x  = element_text(angle=90, vjust=0.5, size=12),
        axis.title.x  = element_text(size=12),
        legend.position = "none")
